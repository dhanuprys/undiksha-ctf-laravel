<script module lang="ts">
    ...
    export const layout = {
        breadcrumbs: [{ title: 'Tantangan', href: index().url }],
    };
</script>
...
<script lang="ts">
    ...
    $effect(() => {
        layout.breadcrumbs = [
            { title: 'Tantangan', href: index().url },
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="lg:col-span-2 space-y-8">
            ...
            >
                ...
                <CardHeader
                    class="border-b border-border/40 pb-8 pt-10 bg-muted/10 relative"
                >
                    <div
                    ...
                    >
                        ...
                        <div
                            class="flex shrink-0 items-center justify-center h-20 w-20 sm:h-24 sm:w-24 rounded-2xl bg-primary/10 border border-primary/20 shadow-inner"
                        >
                            <div
                    ...
                    <div
                        class="flex flex-wrap items-center gap-3 text-sm font-medium mt-6"
                    >
                        <div
                            class="flex items-center gap-2 bg-background px-3.5 py-1.5 rounded-full border border-border/60 shadow-sm text-muted-foreground"
                        >
                            <FolderOpen class="h-4 w-4 text-primary/80" />
                            <span
                                >Kategori: <span class="text-foreground"
                                    >{challenge.category?.name}</span
                                ></span
                            >
                        </div>
                        {#if challenge.solve_count !== null}
                            ...
                            >
                                ...
                                <span
                                    >Diselesaikan <span class="text-foreground"
                                        >{challenge.solve_count}</span
                                    > tim</span
                                >
                            </div>
                        {/if}
                ...
                <CardContent class="p-8">
                    ...
                    {#if challenge.attachments && challenge.attachments.length > 0}
                        <div class="mt-8 pt-8 border-t border-border/40">
                            <div
                                class="flex items-center gap-2 mb-4 text-base font-bold tracking-tight text-foreground"
                            >
                                <Download
                            ...
                            <div class="grid gap-4 sm:grid-cols-2">
                                {#each challenge.attachments as attachment (attachment.id)}
                                    ...
                                    >
                                        <div
                                            class="flex h-12 w-12 shrink-0 items-center justify-center rounded-lg bg-muted border border-border/50 text-muted-foreground group-hover:text-primary group-hover:border-primary/30 group-hover:bg-background transition-colors shadow-sm"
                                        >
                                            <Download
                                                class="h-5 w-5 group-hover:-translate-y-0.5 transition-transform"
                                            />
                                        </div>
                                        <div class="flex flex-col min-w-0">
                                            ...
                                            >
                                            <span
                                                class="text-xs text-muted-foreground font-medium mt-0.5"
                                                >Unduh file</span
                                            >
                                        </div>
                                    </a>
        ...
        <div class="lg:col-span-1">
            <div class="sticky top-24">
                ...
                >
                    ...
                    <CardContent
                        class={`p-6 ${challenge.solved_by_team ? 'pt-2' : ''}`}
                    >
                        {#if challenge.solved_by_team}
                            <div
                                class="flex flex-col items-center text-center space-y-5 py-4"
                            >
                                <div
                                    class="relative flex h-24 w-24 items-center justify-center rounded-full bg-green-500/10 border-2 border-green-500/30 shadow-inner"
                                >
                                    <div
                                        class="absolute inset-0 rounded-full bg-green-500/20 opacity-75"
                                    ></div>
                                    <CheckCircle2
                                        class="h-12 w-12 text-green-600 dark:text-green-500 relative z-10"
                                    />
                                </div>
                                <div class="space-y-1.5">
                                    <h3
                                        class="font-black text-2xl tracking-tight text-green-600 dark:text-green-400"
                                    >
                                        Tepat Sekali!
                                    </h3>
                                    <p class="text-sm text-muted-foreground">
                                        Tim Anda telah menyelesaikan tantangan
                                        ini dan mendapatkan <span
                                            class="font-bold text-foreground"
                                            >{challenge.points_awarded ??
                                                challenge.base_score} poin</span
                                        >.
                                    </p>
                                </div>
                            </div>
